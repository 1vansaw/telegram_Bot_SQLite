...
class GenericCalendar:
    ...
    def __init__(
        ...
        if locale:
            # Локаль ру есть? Подставляем свои массивы
            if locale.startswith("ru"):
                self._labels.days_of_week = ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"]
                self._labels.months = ["Январь", "Февраль", "Март", "Апрель", "Май",
                ...
                try:
                    ...
                    # Если локаль не поддерживается на сервере
                    self._labels.days_of_week = ["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"]
                    self._labels.months = ["January", "February", "March", "April", "May",
